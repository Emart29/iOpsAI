...
class DataStorage:
    ...
    def save_analysis(self, session_id: str, analysis_type: str, results: Dict[str, Any]) -> bool:
        """Save an analysis result"""
        try:
            conn = sqlite3.connect(str(self.db_path))
            
            conn.execute('''
                INSERT INTO analyses (session_id, analysis_type, results)
                VALUES (?, ?, ?)
            ''', (session_id, analysis_type, json.dumps(results)))
